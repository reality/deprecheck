/*
f this file are subject to the LGPL License, Version 3.0.
 ...
 */
public class BlackBoxExplanationFix extends SingleExplanationGeneratorImpl
    ...
    private static final Logger LOGGER =
        LoggerFactory.getLogger(BlackBoxExplanationFix.class.getName());
    /**
     * The Constant DEFAULT_FAST_PRUNING_WINDOW_SIZE.
     */
    private static final int DEFAULT_FAST_PRUNING_WINDOW_SIZE = 10000;
    /**
     ...
     */
    public BlackBoxExplanationFix(OWLOntology ontology, OWLReasonerFactory reasonerFactory,
    ...
    private int expandWithDefiningAxioms(OWLEntity obj, int limit) {
        ...
        getOntology().importsClosure().each { ont -> 
    ...
    private void performFastPruning(OWLClassExpression unsatClass) throws OWLException {
        ...
        LOGGER.info("    - FAST PRUNE ROUND wc:${windowCount}, cw:${currentWindow}");
            axiomWindow.clear();
    ...
    private boolean isSatisfiable(OWLClassExpression unsatClass) throws OWLException {
        ...
        try {
            ...
            LOGGER.info("satisfiable check done: " + sat);
            reasoner.dispose();
            return sat;
    ...
    private void removeDeclarations() {
        debuggingAxioms = asUnorderedSet(
              debuggingAxioms.findAll { ax -> !(ax instanceof OWLDeclarationAxiom) }.stream() );
    }
